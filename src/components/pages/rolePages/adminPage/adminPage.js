...
const AdminPage = () => {
  ...
  const handleRegistrationChange = (e) => {
    if (e.target.name === "photo") {
      ...
      console.log(registrationData); // Здесь вы не увидите обновленное состояние немедленно, потому что setState - асинхронный процесс
    } else {
      setRegistrationData({
        ...registrationData,
        [e.target.name]: e.target.value
      });
      console.log(registrationData); // Здесь также вы не увидите обновленное состояние немедленно
    }
  };
  ...
  const handleRegistrationClient = async () => {
    await registerClient(registrationDataClient, fetchData, cancelSort);
  };
              ...
              {showDeletedBlock && <div style={{ marginLeft: "1.5%" }}>
                ...
                {closeClients.map((client) => (
                  ...
                  >
                    ...
                    <LeedInfo handleDeleteNotice={handleDeleteNotice} closeNoticeEdit={closeNoticeEdit} leedNotice={leedNotice} editNoticeMode={editNoticeMode} setSelectedNotice={setSelectedNotice} handleNoticeClick={handleNoticeClick} handleEditNotice={handleEditNotice} selectedNotice={selectedNotice} dataNotices={dataNotices} handleRegistrationNoticeChange={handleRegistrationNoticeChange} registrationDataNotice={registrationDataNotice} handleRegistrationNotice={handleRegistrationNotice} handleClientClick={handleClientClick} registrationDataClient={registrationDataClient} handleRegistrationClientChange={handleRegistrationClientChange} dataProducts={dataProducts} adminkey={adminKey} editClientViewMode={editClientViewMode} setSelectedClient={setSelectedClient} selectedClient={selectedClient} handleEditClient={handleEditClient} managerIDOptions={managerIDOptions} ManagerStatusOptions={ManagerStatusOptions} close={close} />
